...
export function PlaceScreen({ onBack, onOpenComments, onWriteReview, userId, venueId }: { onBack: () => void; onOpenComments: (reviewId: string) => void; onWriteReview: () => void; userId: string; venueId: string }) {
  ...
  async function reactToReview(reviewId: string) {
    ...
    try {
      const result = await api.reactToReview({ idempotencyKey: createIdempotencyKey('place-review-reaction'), reviewId, reaction: 'like' });
      setReactedReviews((current) => {
        const next = new Set(current);
        if (result.data.active) next.add(reviewId);
  ...
  return <View style={styles.ratingStars} accessibilityLabel={`${clamped.toFixed(1)} out of 5 stars`}>
    {[1, 2, 3, 4, 5].map((star) => {
      ...
      return <View key={star} style={styles.ratingStarCell}>
        <Text style={[styles.stars, styles.emptyStar]}>★</Text>
        {fill > 0 ? <View style={[styles.ratingStarFill, { width: `${fill * 100}%` }]}><Text style={styles.stars}>★</Text></View> : null}
      </View>;
    })}
  </View>;
}

function ReviewCard({ item: review, onComments, onReact, onShare, reacted, reactingId, styles }: { item: NonNullable<ReturnType<typeof usePlaceReviews>['data']>[number]; onComments: (reviewId: string) => void; onReact: (reviewId: string) => void; onShare: (review: NonNullable<ReturnType<typeof usePlaceReviews>['data']>[number]) => void; reacted: boolean; reactingId: string | null; styles: ReturnType<typeof createStyles> }) {
  const { colors } = useAppTheme();
  return <View style={[styles.review, styles.reviewListCard]}>
      ...
      <View style={styles.reviewBody}>
        ...
        {review.dishNames.length > 0 ? <Text style={styles.dishes}>{review.dishNames.join('  ·  ')}</Text> : null}<View style={styles.metricActions}><Pressable disabled={reactingId === review.id} onPress={() => onReact(review.id)} style={styles.metricIconRow}>{reacted ? <HeartBoldIcon color={colors.primary} fill={colors.primary} height={14.3462} width={16.6667} /> : <HeartIcon color={colors.text} height={15.5978} width={17.9167} />}<Text style={styles.metrics}>{review.reactionCount}</Text></Pressable><Pressable onPress={() => onComments(review.id)} style={styles.metricIconRow}><ChatIcon color={colors.text} height={20} width={20} /><Text style={styles.metrics}>{review.commentCount}</Text></Pressable><Pressable onPress={() => onShare(review)} style={styles.metricIconRow}><ShareIcon color={colors.text} height={20} width={20} /></Pressable></View>
      </View>
  </View>;
}
