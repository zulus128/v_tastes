...
Authenticated server reads remain behind Callable Functions. TanStack Query is the mobile server-state owner and persists successful query data in AsyncStorage for up to 24 hours. Query keys include the Firebase UID, and logout clears both memory and disk caches. Offline users may read cached feed, comments, and leaderboard data; mutations remain online-only.
